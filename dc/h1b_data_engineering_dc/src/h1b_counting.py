...
with open(dir + '/input/h1b_input.csv', encoding='utf-8') as csvfile:
    ...
    for row in csv.reader(csvfile, delimiter=';'):
        try: # Have to do this because indexes are not the same for every file -- we have to go by their name!
            ...
            try:
                status_index = row.index("STATUS") # This is saying, find out what the index is for this value
                occupation_index = row.index("LCA_CASE_JOB_TITLE")
                state_index = row.index("LCA_CASE_EMPLOYER_STATE")
            except ValueError:
...
for occ, i in occupation.most_common(10): # i is not index, it's just the second part of the tuple
